...
export function addEventListeners() {
  Element.menuProfile.addEventListener("click", async () => {
    history.pushState(null, null, Route.routePathnames.USERPROFILE);
...
export async function user_profile_page() {
  ...
  <form class="form-profile" method="post">
    ...
    <tr>
        ...
        <input type="text" name="state" value="${
          accountInfo.state
        }" placeholder="State (uppercase 2 letter satate code)" disabled required pattern="[A-Z]+" minlength="2" maxlength="2">
        </td>
        <td>${actionButtons()}</td>
    </tr>
    </table>
  ...
  <form class="form-profile" method="post">
    ...
    <tr>
        ...
        <input type="text" name="zip" value="${
          accountInfo.zip
        }" placeholder="5 digit zip code" disabled required pattern="[0-9]+" minlength="5" maxlength="5">
        </td>
        <td>${actionButtons()}</td>
    </tr>
    </table>
  ...
  <form class="form-profile" method="post">
    ...
    <tr>
        ...
        <td width="60%">
        <input type="text" name="creditNo" value="${
          accountInfo.creditNo
        }" placeholder="credit card number 16 digits" disabled required pattern="[0-9]+" minlength="16" maxlength="16">
        </td>
        <td>${actionButtons()}</td>
    </tr>
    </table>
  ...
  const updateProfilePhotoButton = document.getElementById(
  ...
  updateProfilePhotoButton.addEventListener("click", async () => {
    ...
    const label = Util.disabledButton(updateProfilePhotoButton);
    try {
      const photoURL = await FirebaseController.uploadProfilePhoto(
        photoFile,
        Auth.currentUser.uid
      );
      await FirebaseController.upddateAccountInfo(Auth.currentUser.uid, {
        photoURL,
      });
  ...
  document
    .getElementById("profile-photo-upload-button")
  ...
  for (let i = 0; i < forms.length; i++) {
    forms[i].addEventListener("submit", async (e) => {
      ...
      if (buttonLabel == "Edit") {
        ...
        try {
          await FirebaseController.upddateAccountInfo(
            Auth.currentUser.uid,
            updateInfo
          );
...
export async function getAccountInfo(user) {
  try {
    ...
    if (Constant.DEV) console.log(e);
    Util.info(
      `Failed to retrieve account info for ${user.email}`,
      JSON.stringify(e)
    );
    accountInfo = null;
    return;
